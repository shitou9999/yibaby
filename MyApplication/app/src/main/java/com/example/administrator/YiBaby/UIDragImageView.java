 ...
 * 
 */
public class UIDragImageView extends ImageView {
...
//	private DisplayImageOptions options;
	...
	/** 构造方法 **/
	public UIDragImageView(Context context) {
		super(context);
		init();
	}
	void init()
...
//		.build();
	...
	public UIDragImageView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	...
	public void setImageUrl(String url) {
		if (url != null && !url.equalsIgnoreCase("")) {
			// TODO 组装图片地址
			url = JEREHCommImageTools.realWholeImageUrl(url);
			mPicUrl = url;
			loadImage(url);
		}else
		{
	...
	protected void loadImage(String url) {// 地址为空，返回
		if (url == null || url.equals("")) {
			loadOverCallback(2);
			return;
		} else {
			try {
				imageLoader.displayImage(url, UIDragImageView.this, options);
//				if (null != Constans.IMAGE_OBJECT_MAP.get(url)&& Constans.IMAGE_OBJECT_MAP.get(url).get() != null
//						&& !Constans.IMAGE_OBJECT_MAP.get(url).get().isRecycled()) 
//				{
//					setBitmap(Constans.IMAGE_OBJECT_MAP.get(url).get());
//				} else 
//				{
//					// TODO 从本地内存中取，防止任何取用的地方因为获取图片资源而浪费时间
//					if (JEREHImageViewCache.getInstance().isBitmapExit(url)) 
//					{
//						Bitmap bitmap = JEREHImageViewCache.getInstance().get(url);
//						// TODO 存入内存中
//						if (null != bitmap && !bitmap.isRecycled()) 
//						{
//							Constans.IMAGE_OBJECT_MAP.put(url,new WeakReference<Bitmap>(bitmap));
//						}
//						setBitmap(bitmap);
//					} else 
//					{
...
//				}
			...
			{
				LoadOverPlayer loadOverPlayer=new LoadOverPlayer(2);
				((Activity) context).runOnUiThread(loadOverPlayer);
			} catch (OutOfMemoryError e) 
			{
				LoadOverPlayer loadOverPlayer=new LoadOverPlayer(2);
				((Activity) context).runOnUiThread(loadOverPlayer);
				Log.i("gino", " uiimageview OOM");
				if(processImgError==null)
				{
					processImgError=new CommonProcessImgError(this);
				}
				processImgError.processing(url);
			}
		}
		LoadOverPlayer loadOverPlayer=new LoadOverPlayer(1);
		((Activity) context).runOnUiThread(loadOverPlayer);
	}
	*/
...
//				bmp = getBitmapFromInputStream(bmp, url,1);
//				if (bmp != null) {
//					JEREHImageViewCache.getInstance().put(mPicUrl, bmp, true);
//					Constans.IMAGE_OBJECT_MAP.put(mPicUrl,new WeakReference<Bitmap>(bmp));
...
//					processImgError=new CommonProcessImgError(this);
//				}
//				processImgError.processing(StringUrl);
//			}
//		}
//	}
	...
	class BitmapDisplayer implements Runnable
	{
		...
		public BitmapDisplayer(Bitmap b) {
			bitmap = b;
		}
